...
在支持 Agent Skills 且允许启动本地进程和浏览器的 Codex、Claude Code 或 CodeBuddy 中安装本 Skill 后，可以直接说：

```text
打开 resume-skill-arena，我要用 3 个 Skill 对比修改这份简历。
```

宿主应自动安装依赖、启动本地工作台并打开浏览器；用户不需要另外要求“把网站打开”。如果宿主只能读取 Skill 文本、不能启动本地进程或控制浏览器，就无法提供自动启动，只能按“手动安装与启动”操作。进入工作台后：
